...
- *项目结果:* 提出了BedNet-IR网络，利用高层特征和显著性检测、边缘检测多任务学习方法实现叶片由于曝光轮廓被隐去的良好边缘预测，同时本网络在公开数据集也取得良好的边缘检测结果，展示出了较好的泛化性能。

#item(


  [ *相似航空机加件细粒度分类  * —— 东北大学 / 沈阳飞机工业集团  ],
...
- *项目内容:* 设计相机支架、工件放置平台，对相机、镜头、光源进行选型，完成图像采集平台的硬件设计与平台搭建，使用C\#调用Mech-Eye Nano相机实现机加件的二维+三维信息采集并利用C\#对海康威视Visionmaster软件进行二次开发，设计二维+三维分类算法。
